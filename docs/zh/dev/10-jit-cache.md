...
缓存条目按内容标识组织，实际产物路径请读取 `compiled.output_dir`。

匹配调用复用进程内对象，后续进程校验磁盘产物后复用。源码、特化或工具链标识变化会选择新条目。
磁盘条目缺失或无效时重新构建；无效条目可能回退到私有构建，不覆盖不可变存储。
对象仍在使用时，不应修改或删除其文件。图捕获继续直接使用 Worker 持有的注册记录，不查磁盘。

设置 `PYPTO_CACHE=0` 或 `CacheConfig(enabled=False)` 可关闭持久化，同一进程内仍可复用兼容对象；
此时更换输出父目录会选择另一个私有对象。显式固定的 `output_dir=` / `save_kernels_dir=`
请求和编译诊断仍会重新编译。
