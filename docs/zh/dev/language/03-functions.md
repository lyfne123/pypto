...
- 显式指定 `save_kernels_dir` 或 `CompileOptions(output_dir=...)`。
- 显式诊断设置，或包含 instrument、验证/诊断设置不同于流水线默认值的活跃
  `PassContext`。

`PYPTO_PROG_BUILD_DIR` 只选择输出位置，不强制重编。默认复用进程内对象；显式开启持久缓存后，在该父目录的
`.pypto-cache` 下保存产物，除非缓存策略指定其他根目录。
优先级及开启方式见[缓存配置](../10-jit-cache.md#配置)。
...
动态维度沿用现有编译规则。`pl.constexpr` 参数是例外：它的值决定选中哪份产物，
因此必须通过关键字或签名默认值给出。
调用返回的编译对象时，需提供包含标量实参在内的完整参数列表——每个标量的值在此处读取；
constexpr 参数不在该列表中，产物已经把它解析掉了。
